...
export default async function CampaignPage({ params }: { params: Params }) {
  ...
  return (
    ...
    >
      ...
      <h1 className="max-w-md text-4xl font-bold tracking-tight text-(--brand)">
        {campaign.title}
      </h1>

      <Link
        href={`/${campaign.slug}/cadastro`}
        className="rounded-full bg-(--brand) px-8 py-3 text-base font-medium text-white transition-opacity hover:opacity-90"
      >
        Participar
      </Link>
    </div>
